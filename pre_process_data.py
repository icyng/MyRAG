...
- 表や図などを無理やりOCRまたは手動でテキスト化したようなデータを機械が翻訳できるよう前処理する
- 入力された非構造化テキストに対し、半構造化データ("【{section}】\n{chunk}"こんな感じ)にしてモデルに渡す役目を果たす

# 注意事項

- OCRテキストを PreProcessing で走らせる際は、以下の事前処理を行っていただくことに留意：
    - テキストにおけるセクションの見出しは`【見出し】`形式に修正し、かつ先頭に`\n\n`を2つ挿入すること（分割箇所を明確にするため）
    - 問題文中の「ア」のように空欄部を表す単語については、あらかじめ` [ア] `のように「角括弧 + 前後の空白」に修正しておくこと
    - 問題文中の傍線部で表される部分については、あらかじめ鉤括弧`{}`を用いた表示で囲むように修正しておくこと
    - 適宜、問題冊子のPDFデータをテキスト化した際に発生する不要な改行や全角空白を除去しておくこと（不要な場合もあり）
"""
